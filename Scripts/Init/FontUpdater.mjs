...
function setup()
{
	...
	{
		if (PreserveHeights.length > 0)
		{
			let code = MOV(EAX, [ESP, 4]);
			for (const h of PreserveHeights)
				code += CMP(EAX, h) + JE(Filler(1,1));
			code += MOV([ESP, 4], CACHE.get('FONT_Height'));
			code = SetFillTargets(code, {'1,1' : code.byteCount()});
			parts.push(code);
		}
		else
		{
			parts.push(
				MOV([ESP, 4], CACHE.get('FONT_Height'))
			);
		}
	}
...
export function setPreserveHeights(arr)
{
	PreserveHeights = (arr && arr.length) ? Array.from(new Set(arr)) : [];
}
